...
# Defines analytics for Invoices
module InvoiceAnalytics
  ...
  def top_days_by_invoice_count
    invoices = count_days invoice_days
    days_array = invoices_by_day invoices

    avg = days_array.reduce(:+) / days_array.length.to_f
    deviation = StandardDeviation.calculate days_array

    days_with_invoice_count_twice_above invoices, avg, deviation
  end

  def days_with_invoice_count_twice_above(invoices, avg, deviation)
    invoices.select do |_day, num_invoices|
